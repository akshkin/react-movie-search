...
function Navigation() {
  ...
  const { option, setOption, currentPage, searchQuery, searchTerm } =
    useContext(Context);

  const reqLocation =
    location.pathname === "/" || location.pathname.split("/")[1] === `search`;
      ...
      {reqLocation && (
        <ul>
          {options.map((thisOption, index) => (
            <li
              key={index}
              className={thisOption === option ? "active" : ""}
              onClick={() => setOption(thisOption)}
            >
